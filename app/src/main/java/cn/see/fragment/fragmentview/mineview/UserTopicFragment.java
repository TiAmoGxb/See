...
import cn.see.presenter.minep.UserTopicPresenter;
...
public class UserTopicFragment extends BaseFragement<UserTopicPresenter>implements  PullToRefreshBase.OnRefreshListener2<ListView> {
    ...
    public static UserTopicFragment newInstance(int type) {
        UserTopicFragment newFragment = new UserTopicFragment();
    ...
    public void initAfter() {
        if (type == 0){
            getP().getTopEdit(page);
        }else{
            getP().getTopApply(page);
        }
    }
    ...
    @Override
    public UserTopicPresenter bindPresent() {
        return new UserTopicPresenter();
    }
