...
import cn.see.chat.pickerimage.utils.AttachmentStore;
import cn.see.chat.pickerimage.utils.StorageUtil;
import cn.see.chat.utils.BitmapLoader;
import cn.see.chat.utils.DialogCreator;
import cn.see.chat.utils.NativeImageLoader;
import cn.see.chat.view.ImgBrowserViewPager;
import cn.see.chat.view.PhotoView;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(App.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(App.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(App.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(App.POSITION, 0);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(App.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(App.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(App.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(App.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(App.MsgIDs, activity.mMsgIds);
                        activity.setResult(App.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
