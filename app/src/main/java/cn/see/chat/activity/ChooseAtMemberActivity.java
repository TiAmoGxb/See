...
import cn.see.chat.adapter.AtMemberAdapter;
import cn.see.chat.model.ParentLinkedHolder;
import cn.see.chat.utils.keyboard.widget.EmoticonsEditText;
import cn.see.chat.utils.pinyin.UserComparator;
import cn.see.chat.utils.sidebar.SideBar;
import cn.see.chat.view.listview.StickyListHeadersListView;
...
public class ChooseAtMemberActivity extends BaseActivity {
    ...
    public static void show(ChatActivity textWatcher, EmoticonsEditText editText, String targetId) {
        ...
        intent.putExtra(App.GROUP_ID, Long.parseLong(targetId));
        textWatcher.startActivityForResult(intent, App
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = getIntent().getLongExtra(App.GROUP_ID, 0);
        if (0 != groupId) {
        ...
        mLl_groupAll.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                //@all
                Intent intent = new Intent();
                intent.putExtra(App.ATALL, true);
                setResult(App.RESULT_CODE_AT_ALL, intent);
                finish();
            }
        });
        ...
        mSearch_title.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                App.mSearchAtMember = mList;
                startActivityForResult(intent, App.SEARCH_AT_MEMBER_CODE);
            }
        });
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 33 && data != null) {
            Intent intent = new Intent();
            intent.putExtra(App.NAME, data.getStringExtra(App.SEARCH_AT_MEMBER_NAME));
            intent.putExtra(App.TARGET_ID, data.getStringExtra(App.SEARCH_AT_MEMBER_USERNAME));
            intent.putExtra(App.TARGET_APP_KEY, data.getStringExtra(App.SEARCH_AT_APPKEY));
            setResult(App.RESULT_CODE_AT_MEMBER, intent);
            finish();
        }
    }
