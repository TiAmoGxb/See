...
import cn.see.chat.adapter.ChattingListAdapter;
import cn.see.chat.entity.Event;
import cn.see.chat.entity.EventType;
import cn.see.chat.location.activity.MapPickerActivity;
import cn.see.chat.model.Constants;
import cn.see.chat.pickerimage.PickImageActivity;
import cn.see.chat.pickerimage.utils.Extras;
import cn.see.chat.pickerimage.utils.RequestCode;
import cn.see.chat.pickerimage.utils.SendImageHelper;
import cn.see.chat.pickerimage.utils.StorageType;
import cn.see.chat.pickerimage.utils.StorageUtil;
import cn.see.chat.pickerimage.utils.StringUtil;
import cn.see.chat.utils.IdHelper;
import cn.see.chat.utils.SharePreferenceManager;
import cn.see.chat.utils.SimpleCommonUtils;
import cn.see.chat.utils.ToastUtil;
import cn.see.chat.utils.event.ImageEvent;
import cn.see.chat.utils.imagepicker.bean.ImageItem;
import cn.see.chat.utils.keyboard.XhsEmoticonsKeyBoard;
import cn.see.chat.utils.keyboard.data.EmoticonEntity;
import cn.see.chat.utils.keyboard.interfaces.EmoticonClickListener;
import cn.see.chat.utils.keyboard.utils.EmoticonsKeyboardUtils;
import cn.see.chat.utils.keyboard.widget.EmoticonsEditText;
import cn.see.chat.utils.keyboard.widget.FuncLayout;
import cn.see.chat.utils.photovideo.takevideo.CameraActivity;
import cn.see.chat.view.ChatView;
import cn.see.chat.view.SimpleAppsGridView;
import cn.see.chat.view.TipItem;
import cn.see.chat.view.TipView;
import cn.see.chat.view.listview.DropDownListView;
...
public class ChatActivity extends BaseActivity implements FuncLayout.OnFuncKeyBoardListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        cn.see.base.BaseActivity.setTranslucentStatus(false,this);

        if (Build.VERSION.SDK_INT <Build.VERSION_CODES.M) {
            cn.see.base.BaseActivity.setStatusBarTo(getResources().getColor(R.color.black),getWindow());
        }
        setContentView(R.layout.activity_chat);
        mChatView = (ChatView) findViewById(R.id.chat_view);
        RelativeLayout bacRela = (RelativeLayout) findViewById(R.id.back_rela);
        bacRela.setOnClickListener(this);
    ...
    private void initData() {
        ...
        mTitle = intent.getStringExtra(App.CONV_TITLE);
...
//        mChatAdapter.initMediaPlayer();
    ...
    public void startChatDetailActivity(String targetId, String appKey, long groupId) {
        ...
        startActivityForResult(intent, App.REQUEST_CODE_CHAT_DETAIL);
    }
    ...
    protected void onResume() {
        ...
        if (!mIsSingle) {
            ...
            if (groupId != 0) {
                App.isAtMe.put(groupId, false);
                App.isAtall.put(groupId, false);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        switch (resultCode) {
            case App.RESULT_CODE_AT_MEMBER:
                if (!mIsSingle) {
                    GroupInfo groupInfo = (GroupInfo) mConv.getTargetInfo();
                    String username = data.getStringExtra(App.TARGET_ID);
                    String appKey = data.getStringExtra(App.TARGET_APP_KEY);
                    ...
                    mLongClick = true;
                    ekBar.getEtChat().appendMention(data.getStringExtra(App.NAME));
                    ekBar.getEtChat().setSelection(ekBar.getEtChat().getText().length());
                }
                break;
            case App.RESULT_CODE_AT_ALL:
                mAtAll = data.getBooleanExtra(App.ATALL, false);
                mLongClick = true;
                if (mAtAll) {
            ...
            case RequestCode.TAKE_VIDEO:
                ...
                break;
            case App.RESULT_CODE_SEND_LOCATION:
                ...
                mChatView.setToBottom();
                break;
            case App.RESULT_CODE_SEND_FILE:
                int[] intArrayExtra = data.getIntArrayExtra(MsgIDs);
                for (int msgId : intArrayExtra) {
                    handleSendMsg(msgId);
                }
                break;
            case App.RESULT_CODE_CHAT_DETAIL:
                String title = data.getStringExtra(App.CONV_TITLE);
                if (!mIsSingle) {
                    GroupInfo groupInfo = (GroupInfo) mConv.getTargetInfo();
                    UserInfo userInfo = groupInfo.getGroupMemberInfo(mMyInfo.getUserName(), mMyInfo.getAppKey());
                    //如果自己在群聊中，同时显示群人数
