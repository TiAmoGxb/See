...
import cn.see.chat.utils.SortConvList;
import cn.see.chat.utils.SortTopConvList;
import cn.see.chat.utils.ThreadUtil;
import cn.see.chat.utils.TimeFormat;
import cn.see.chat.utils.ViewHolder;
import cn.see.chat.view.ConversationListView;
import cn.see.chat.view.SwipeLayoutConv;
...
//                //会话界面时间
                ...
                if (mAtAll.get(position) && App.isAtall.get(gid) != null && App.isAtall.get(gid)) {
                ...
                } else if (mArray.get(position) && App.isAtMe.get(gid) != null && App.isAtMe.get(gid)) {
                    //有人@我 文字提示
                    ...
                    if (lastMsg.getTargetType() == ConversationType.group && !contentStr.equals("[群成员变动]")) {
                        ...
                        if (App.isAtall.get(gid) != null && App.isAtall.get(gid)) {
                            content.setText("[@所有人] " + fromName + ": " + contentStr);
                        } else if (App.isAtMe.get(gid) != null && App.isAtMe.get(gid)) {
        ...
        if (convItem.getType().equals(ConversationType.single)) {
            ...
            Log.i(TAG,"convItem.getTitle():"+convItem.getTitle());
            Log.i(TAG,"mUserInfo:"+mUserInfo);
            if (mUserInfo != null) {
                mUserInfo.getAvatarBitmap(new GetAvatarBitmapCallback() {
                    @Override
                    public void gotResult(int status, String desc, Bitmap bitmap) {
                        if (status == 0) {
                            GlideDownLoadImage.getInstance().loadBitmapCircleImage(headIcon,bitmap);
                        } else {
                            GlideDownLoadImage.getInstance().loadCircleImage(R.drawable.bac_image, headIcon);                        }
                    }
                });
            } else {
                GlideDownLoadImage.getInstance().loadCircleImage(R.drawable.bac_image, headIcon);
