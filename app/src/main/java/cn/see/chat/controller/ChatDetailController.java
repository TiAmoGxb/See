...
import cn.see.chat.activity.ChatDetailActivity;
import cn.see.chat.activity.FriendInfoActivity;
import cn.see.chat.activity.GroupAvatarActivity;
import cn.see.chat.activity.GroupGridViewActivity;
import cn.see.chat.activity.GroupNotFriendActivity;
import cn.see.chat.activity.MainActivity;
import cn.see.chat.activity.MembersInChatActivity;
import cn.see.chat.activity.PersonalActivity;
import cn.see.chat.activity.VerificationActivity;
import cn.see.chat.activity.historyfile.activity.HistoryFileActivity;
import cn.see.chat.adapter.GroupMemberGridAdapter;
import cn.see.chat.database.FriendEntry;
import cn.see.chat.database.FriendRecommendEntry;
import cn.see.chat.entity.Event;
import cn.see.chat.entity.EventType;
import cn.see.chat.utils.DialogCreator;
import cn.see.chat.utils.ToastUtil;
import cn.see.chat.view.ChatDetailView;
import cn.see.chat.view.SlipButton;
...
public class ChatDetailController implements OnClickListener, OnItemClickListener,
    ...
    public void initData() {
        ...
        mGroupId = intent.getLongExtra(App.GROUP_ID, 0);
        mTargetId = intent.getStringExtra(App.TARGET_ID);
        mTargetAppKey = intent.getStringExtra(App.TARGET_APP_KEY);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.return_btn:
                ...
                intent.putExtra(App.CONV_TITLE, getName());
                intent.putExtra(App.MEMBERS_COUNT, mMemberInfoList.size());
                mContext.setResult(App.RESULT_CODE_CHAT_DETAIL, intent);
                mContext.finish();
                break;
            ...
            case R.id.tv_moreGroup:
                ...
                intent.putExtra(App.DELETE_MODE, false);
                mContext.startActivityForResult(intent, App.REQUEST_CODE_ALL_MEMBER);
                break;
                                        ...
                                        if (content.getContentType() == ContentType.image) {
                                            ...
                                            if (!TextUtils.isEmpty(localPath)) {
                                                ...
                                                if (file.exists()) {
                                                    ...
                                                    File copyFile = new File(App.FILE_DIR + fileContent.getFileName());
            ...
            mUserInfo.removeFromFriendList(new BasicCallback() {
                ...
                public void gotResult(int responseCode, String responseMessage) {
                    ...
                    if (responseCode == 0) {
                        ...
                        App.forAddFriend.remove(mUserInfo.getUserName());
                        //将好友删除时候还原黑名单设置
    ...
    public void onItemClick(AdapterView<?> viewAdapter, View view, final int position, long id) {
        ...
        if (mIsGroup) {
            ...
            if (position < mCurrentNum) {
                ...
                intent.putExtra(App.DELETE_MODE, true);
                intent.putExtra(App.GROUP_ID, mGroupId);
            ...
            mContext.startActivityForResult(intent, App.REQUEST_CODE_FRIEND_INFO);
        } else if (position == mCurrentNum) {
