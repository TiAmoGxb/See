...
import cn.see.chat.activity.SendFileActivity;
import cn.see.chat.activity.fragment.AudioFragment;
import cn.see.chat.activity.fragment.DocumentFragment;
import cn.see.chat.activity.fragment.ImageFragment;
import cn.see.chat.activity.fragment.OtherFragment;
import cn.see.chat.activity.fragment.VideoFragment;
import cn.see.chat.adapter.ViewPagerAdapter;
import cn.see.chat.entity.FileType;
import cn.see.chat.entity.UpdateSelectedStateListener;
import cn.see.chat.utils.BitmapLoader;
import cn.see.chat.view.SendFileView;
...
public class SendFileController implements View.OnClickListener, ViewPager.OnPageChangeListener,
    ...
    public SendFileController(SendFileActivity context, SendFileView view) {
        ...
        String targetId = mContext.getIntent().getStringExtra(App.TARGET_ID);
        String targetAppKey = mContext.getIntent().getStringExtra(App.TARGET_APP_KEY);
        long groupId = mContext.getIntent().getLongExtra(App.GROUP_ID, 0);
        if (groupId != 0) {
            ...
            if (controller != null) {
                switch (msg.what) {
                    case SEND_FILE:
                        ...
                        intent.putExtra(App.MsgIDs, controller.mMsgIds);
                        controller.mContext.setResult(App.RESULT_CODE_SEND_FILE, intent);
